    ...
    def generate_loss(y):#cross_entropy
        ...
        for k in range(batch_size):
            ...
            for i in range(window):
                ...
                if i==0:
                    pred=tf.reduce_mean(tf.squeeze(tf.slice(y,[k,1,0],[1,window-1,embedding_size])),axis=0,keep_dims=True)
                    context=tf.squeeze(tf.slice(y,[k,1,0],[1,window-1,embedding_size]))
                elif i==(window-1):
                    pred=tf.reduce_mean(tf.squeeze(tf.slice(y,[k,0,0],[1,window-1,embedding_size])),axis=0,keep_dims=True)
                    context=tf.squeeze(tf.slice(y,[k,0,0],[1,window-1,embedding_size]))
                else:
                    pred=tf.reduce_mean(tf.concat([tf.squeeze(tf.slice(y,[k,0,0],[1,i,embedding_size]),[0]),tf.squeeze(tf.slice(y,[k,i+1,0],[1,window-i-1,embedding_size]),[0])],axis=0),axis=0,keep_dims=True)
