...
with graph.as_default():
    ...
    optimizer=tf.train.MomentumOptimizer(learning_rate=lr, momentum=0.99,use_locking=False,name='nesterov',use_nesterov=True)
...
with tf.Session(graph=graph,config=config) as session:
    ...
    for step in range(num_steps_train):
        global_step=step
        w_p1,v1=generate_input(step,train_data,dictionary)
        feed={wi:v1,wi_pvector:w_p1,keep_prob:drop_keep}
        
        _,loss_val=session.run([train_op,cross_entropy],feed_dict=feed)
        
        average_loss+=loss_val
        
        if ((step+1)%175==0)and(step>0):
            average_loss/=175
            ppl=np.exp(average_loss)
            print('Train_loss at%d: %.5f'%((step+1),average_loss),' ppl:%.2f'%ppl)
            average_loss=0
    loss=0
    for step in range(num_steps_valid):
        global_step=step
        w_p1,v1=generate_input(step,valid_data,dictionary)
        feed={wi:v1,wi_pvector:w_p1,keep_prob:drop_keep}
    ...
    for step in range(num_steps_test):
        global_step=step
        w_p1,v1=generate_input(step,test_data,dictionary)
        feed={wi:v1,wi_pvector:w_p1,keep_prob:drop_keep}
        [loss_val]=session.run([cross_entropy],feed_dict=feed) 
        loss+=loss_val   
    loss/=num_steps_test
