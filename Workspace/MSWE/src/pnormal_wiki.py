...
home_dir='/home/liuyang/word_sense_jungle'
save_dir=home_dir+'/logs/save'
log_dir=home_dir+'/logs/summaries'
filename='n4d100w30_wiki_w2v'
#text8_dataset wiki9_dataset
datafile='wiki9_dataset'
#text8 wiki9
dataname='wiki9'
#w2v_vector glove
vectorfile='w2v_vector'
#注意是否为二进制，后面代码要相应修改
vectorname='w=30d=100.bin'
#vocab不用自己生成，用glove生成的vocab直接拷到dataset文件夹中
...
def build_dataset():
    ...
    with open(home_dir+'/%s/vocab.txt'%datafile,'r',encoding='utf-8') as fin:
        count=[(x.rstrip().split(' ')[0],x.rstrip().split(' ')[1]) for x in fin.readlines()]
    #前面是word后面是频率
    dictionary=dict()
    for word,_ in count:
...
#初始化词向量位置p矩阵
w1 = KeyedVectors.load_word2vec_format(home_dir+'/%s/%s'%(vectorfile,vectorname), encoding='utf-8',binary=True)
print('初始化位置词向量矩阵p完毕')
...
with graph.as_default():
    ...
    with tf.name_scope('w_conv2'):
        w_conv2=weight_variable([window+1,embedding_size,1,2*embedding_size],1,drop_keep,(window+1)*embedding_size)
        variable_summaries(w_conv2, 'weight-conv2')
    with tf.name_scope('b_conv2'):
        b_conv2=bias_variable([2*embedding_size],0)
        variable_summaries(b_conv2, 'bias-conv2')
        
    with tf.name_scope('w_conv3'):
        w_conv3=weight_variable([window+1,embedding_size,1,2*embedding_size],1,drop_keep,(window+1)*embedding_size)
        variable_summaries(w_conv3, 'weight-conv3')
    with tf.name_scope('b_conv3'):
        b_conv3=bias_variable([2*embedding_size],0)
        variable_summaries(b_conv3, 'bias-conv3')
        
    with tf.name_scope('w_conv4'):
        w_conv4=weight_variable([window+1,embedding_size,1,2*embedding_size],1,drop_keep,(window+1)*embedding_size)
        variable_summaries(w_conv4, 'weight-conv4')
    with tf.name_scope('b_conv4'):
        b_conv4=bias_variable([2*embedding_size],0)
        variable_summaries(b_conv4, 'bias-conv4')
    ...
    with tf.name_scope('matrix5-conv'):
        c_pio=tf.transpose(cnn_batch_normal(tf.transpose(attention_gated_cnn(c_pio_drop4,w_conv5,b_conv5,window))))
    ...
    #lr decay
    lr = tf.train.exponential_decay(starter_learning_rate, global_step,500, 0.85, staircase=True)
...
#到命令行执行 tensorboard --logdir=/home/liuyang/word_sense_jungle/logs/summaries
#出来提示信息的网页，复制到浏览器即可
